            ...
            {controls.map((ctrl) => (
              <div key={ctrl.key} className="space-y-2.5">
                <div className="flex justify-between items-center px-1">
                  ...
                  <span className="text-[10px] mono text-indigo-400 font-bold">{typeof (grading as any)[ctrl.key] === 'number' ? (grading as any)[ctrl.key].toFixed(2) : ((grading as any)[ctrl.key] ?? 0)}</span>
                </div>
                <input 
                  type="range" 
                  min={ctrl.min} 
                  max={ctrl.max} 
                  step={ctrl.step} 
                  value={(grading as any)[ctrl.key] ?? 0} // FIX: Added nullish coalescing to prevent uncontrolled input
