...
const ZModeModal: React.FC<ZModeModalProps> = ({ isOpen, onClose, params, setParams }) => {
  ...
  return (
    <motion.div 
      ...
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
      className="fixed inset-0 z-[9999] flex items-center justify-center overflow-hidden p-0 sm:p-4"
    >
      ...
      <motion.div 
        ...
        animate={{ scale: 1, y: 0 }}
        className="relative w-full max-w-2xl h-full md:h-auto max-h-[95vh] md:max-h-[85vh] bg-[#121215] border-x md:border border-white/10 md:rounded-[3rem] overflow-hidden shadow-[0_0_120px_rgba(79,70,229,0.15)] flex flex-col"
      >
        ...
        <div className="flex-1 overflow-y-auto p-6 md:p-10 space-y-10 scroll-smooth pb-24 relative custom-scrollbar">
            ...
            {sliders.map((s) => (
              <div key={s.key} className={`space-y-4 transition-all duration-700 ${params.auto_tune_active ? 'opacity-40 saturate-0' : 'opacity-100'}`}>
                <div className="flex justify-between items-end">
                  ...
                  <span className="text-sm font-black text-indigo-400 mono tracking-tighter">{((params as any)[s.key] ?? 0 * 100).toFixed(1)}%</span>
                </div>
                <input 
                  type="range" 
                  disabled={params.auto_tune_active} 
                  min={s.min} max={s.max} step={s.step} 
                  value={(params as any)[s.key] ?? 0} 
